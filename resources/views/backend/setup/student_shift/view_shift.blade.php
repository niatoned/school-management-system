					  ...
					  <table id="example1" class="table table-bordered table-striped">
                        ...
                        @foreach($allData as $key => $shift)
							<tr>
								<td>{{$key + 1}}</td>
								<td>{{$shift->name}}</td>
								<td><a href="{{ route('student.shift.edit', $shift->id) }}" class="btn btn-info">Edit</a>
                                <a id="delete" href="{{ route('student.shift.delete', $shift->id) }}" class="btn btn-danger">Delete</a></td>
							</tr>
						@endforeach
						</tbody>
						<tfoot>
