					  ...
					  <table id="example1" class="table table-bordered table-striped">
                        ...
                        @foreach($allData as $key => $year)
							<tr>
								<td>{{$key + 1}}</td>
								<td>{{$year->name}}</td>
								<td><a href="{{ route('student.year.edit', $year->id) }}" class="btn btn-info">Edit</a>
                                <a id="delete" href="{{ route('student.year.delete', $year->id) }}" class="btn btn-danger">Delete</a></td>
							</tr>
						@endforeach
						</tbody>
						<tfoot>
